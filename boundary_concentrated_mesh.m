function [p_new,e_new,elem_new] = boundary_concentrated_mesh(p,elem,h,geom,C,max_iter)
% 对现有网格进行边界集中加密
% 输入：
%     p：网格节点坐标 np*2
%     elem：三角形单元 nt*3
...
% 输出：
%     p_new：加密后节点坐标，np * 2
%     e_new：加密后边界节点信息，ne * 2
%     elem_new：加密后三角形单元，nt * 3

bd_seg = boundary_segment(geom);
...
for iter = 1:max_iter
    ...
    hT = elem_diam(p_cur,elem_cur);             % 单元的最大边长
    rho = dist2bd(p_cur,elem_cur,bd_seg);       % 单元到边界的距离

    % 标记待细化单元
    mark_elem = false(size(elem_cur,1),1);
    for i = 1:size(mark_elem,1)
        if rho(i) < 1e-12 && hT(i) > C * h^2
                mark_elem(i) = true;
        end
    end
    ...
    % 细化标记单元
    [p_cur,elem_cur] = bisect(p_cur,elem_cur,find(mark_elem));

end

% 提取边界边信息
T = auxstructure(elem_cur);
e_new = T.bdEdge';
